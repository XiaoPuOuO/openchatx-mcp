...
export function buildMcpInstructions(): string {
  ...
  return (
    ...
    "- summarize is always available after start_here for cross-conversation handoff. Pass summary to store a temporary continuation summary and receive a UUID. In the next ChatGPT conversation, pass that UUID to summarize to retrieve the summary; retrieval consumes and deletes it.\n" +
    ...
    "- Treat Projects as real workspaces. Project tools are always available. At the start of every new user task, decide whether it belongs to a durable Project. Machine/global tasks such as SSH setup, firewall changes, or general OS administration normally do not. For project work, use project_list/project_use/project_manage directly, reuse a matching Project when one exists, and clear a mismatched active Project before discovery. If no Project matches, locate the relevant project folders, choose exactly one primary root plus any additional roots that belong to the same workspace, create the Project with project_manage, and activate it before project-focused work.\n" +
    "- An active Project may contain multiple folder roots but exactly one primary root. Relative file/search/shell paths resolve from the primary root. Absolute paths are limited to that Project's roots. If a tool reports PROJECT_EXTERNAL_ACCESS_REQUIRED, ask the user for permission, then discover/call project_access. Use grant_all_session only when the user explicitly says not to ask again.\n" +
