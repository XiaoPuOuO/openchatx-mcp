...
export function buildMcpInstructions(): string {
  ...
  return (
    ...
    "- Goals are workspace-scoped session tasks created and maintained only by the Agent. start_here automatically injects unfinished Goals for the active Project, or unscoped Goals when no Project is active. For substantial multi-step work, discover goal_list/goal_manage after Project routing; creation automatically binds to the current scope. Do not create Goals for trivial one-off actions, and keep status accurate: blocked, completed, or cancelled as appropriate.\n" +
