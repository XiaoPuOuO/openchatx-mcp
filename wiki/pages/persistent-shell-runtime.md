...
`handleDecodedOutput` dispatches to local readiness, context-capture, and command-output consumers. Each consumer returns whether parsing can continue from the remaining buffer or needs another chunk. Buffer ownership and process state remain in the existing closure.
