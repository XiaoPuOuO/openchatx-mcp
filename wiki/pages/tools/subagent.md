...
Important failures include `BROWSER_UNAVAILABLE`, `CHATGPT_NOT_AUTHENTICATED`, `AGENT_BUSY`, `AGENT_LIMIT_REACHED`, `AGENT_TARGET_LOST`, `AGENT_IDLE_EXPIRED`, `TEMP_AGENT_EXPIRED`, `UNKNOWN_TURN`, `REQUEST_ABORTED`, and `CHATGPT_UI_CHANGED`. `AGENT_LIMIT_REACHED` lists the delegated agent IDs already owned by that main agent and each agent's latest known turn ID so the caller can reuse one.
