...
The next distinct caller becomes `agent-2`, then `agent-3`, and so on for the lifetime of that logger. The alias is presentation-only; raw OpenAI session values are not written to the audit log (`src/server/http-server.ts`, `src/server/audit-log.ts`).
