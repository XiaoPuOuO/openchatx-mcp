...
Shellby MCP uses `X-OpenAI-Subject` as the remote owner identifier and `X-OpenAI-Session` as conversation-scoped operational context (`src/server/http-server.ts`, `src/auth/auth.ts`). Session context scopes completion notices, audit activity, and the process-local `start_here` initialization gate; it is never authorization state and remains unsuitable for durable user binding. OpenAI marks `openai/userAgent` and `openai/userLocation` as best-effort hints that must not be relied on for authorization.

Each MCP request may carry `X-OpenAI-Session`, which Shellby treats as the opaque identity of the ChatGPT conversation making that request. A process-level set records sessions that successfully called `start_here`; before that, the shared tool-registration boundary rejects other tool handlers. Sessions without the header are not startup-gated. Shellby does not infer relationships between sessions or classify a caller as a browser subagent. Audit output maps distinct raw session values to readable `agent-N` aliases (`src/server/http-server.ts`, `src/server/tool-registration-boundary.ts`, `src/server/audit/audit-log.ts`).

## Connection Model

Production HTTP uses the MCP v2 `createMcpHandler(factory)` entry wrapped once with `toNodeHandler` and mounted on the MCP Express app. The factory creates a fresh `McpServer` for each serving unit while all requests share the process-level `ShellSessionManager`, `WebPageOpener`, `PeekabooClient`, `ChatGptSubagentService`, startup-session set, and production `ShellbyAuthStore`. The handler owns request classification, modern per-request serving, response streaming, and server teardown (`src/server/http-server.ts`, `src/server/mcp-server.ts`).
