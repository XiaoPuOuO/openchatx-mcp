...
Each MCP request may carry `X-OpenAI-Session`, which Shellby treats as the opaque identity of the ChatGPT conversation making that request. Shellby does not infer relationships between sessions or classify a caller as a browser subagent. Audit output maps distinct raw session values to readable `agent-N` aliases (`src/server/http-server.ts`, `src/server/audit-log.ts`).

## Connection Model

Production HTTP uses the MCP v2 `createMcpHandler(factory)` entry wrapped once with `toNodeHandler` and mounted on the MCP Express app. The factory creates a fresh `McpServer` for each serving unit while all requests share the process-level `ShellSessionManager`, `WebPageOpener`, `PeekabooClient`, `ChatGptSubagentService`, and production `ShellbyAuthStore`. The handler owns request classification, modern per-request serving, response streaming, and server teardown (`src/server/http-server.ts`, `src/server/mcp-server.ts`).
